...
def create_train_transforms(size=224, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), is_crop=True, random_mask=False, jpeg_quality=None, opt=None):
    """创建训练数据增强转换管道"""
    ...
    transform_list = [
        ...
        RandomJPEGCompression(quality_lower=50, quality_upper=100, p=0.1), 
...
class RealFakeDataset(Dataset):
    ...
    def __init__(self, opt):
        # Validate data label
        assert opt.data_label in ["train", "val"], f"Invalid data_label: {opt.data_label}"
        self.opt = opt
        ...
        for real_path in tqdm(real_list, desc=f"Loading {source_name} dataset..."):
            ...
            sample = {
                ...
                'jpeg_quality': self.real_quality_factor_mapping.get(
                    os.path.basename(real_path), 96  # FIXED: was real_img_path, now real_path
                )
            }
