...
class RandomScaleCropOrDirect224:
    """
    训练模式（train=True）三种策略（按 probs 概率随机挑选）：
        - direct:         直接 Resize 到 224×224（打破纵横比）
        - randcrop:       在原图上 RandomCrop(224×224)（短边不足按 small_image_policy 兜底）
        - scale_then_crop:随机采样短边 S∈short_side_range，等比缩放到短边=S，然后 RandomCrop(224×224)

    推理模式（train=False，policy 同前一版）：
        - 'short256_center'（默认）/ 'resize256_center' / 'direct'
    """

    def __init__(
        self,
        crop_size=224,
        short_side_range=(224, 320),
        probs=(
            0.2,
            0.3,
            0.5,
            0.5,
        ),  # (direct, randcrop, scale_then_crop)
        small_image_policy="resize",  # 'resize' | 'pad' | 'center'
        interpolation=InterpolationMode.BICUBIC,
        antialias=True,
        train=True,
        infer_policy="short256_center",  # 'short256_center' | 'direct' | 'resize256_center'
        eval_resize_short=256,
    ):
        # 基本检查
        assert len(probs) == 4 and all(p >= 0 for p in probs) and sum(probs) > 0, (
            "probs must be 4 non-negative numbers"
        )
        assert short_side_range[0] > 0 and short_side_range[1] >= short_side_range[0]
        assert small_image_policy in ("resize", "pad", "center")

        # 归一化概率
        s = float(sum(probs))
        self.probs = (probs[0] / s, probs[1] / s, probs[2] / s, probs[3] / s)
    ...
    def _resize_short(self, img, target_short: int):
        ...
        return F.resize(
            img, [new_h, new_w], interpolation=self.interp, antialias=self.antialias
        )

    def _direct_resize_224(self, img):
        return F.resize(
            img,
    ...
    def _random_crop_square(self, img, size: int):
        ...
        if w < size or h < size:
            ...
            if self.small_image_policy == "resize":
                ...
                pad_w = max(0, size - w)
                pad_h = max(0, size - h)
                # padding 顺序: left, top, right, bottom
    ...
    def __call__(self, img):
        if self.train:
            mode = random.choices(
                population=("centercrop", "direct", "randcrop", "scale_then_crop"),
                weights=self.probs,
                k=1,
            )[0]

            if mode == "centercrop":
                return F.center_crop(img, [self.crop_size, self.crop_size])

            if mode == "direct":
                return self._direct_resize_224(img)

            if mode == "randcrop":
                # 直接在原图上随机裁 224（不足则按策略兜底）
                return self._random_crop_square(img, self.crop_size)

            # mode == "scale_then_crop"
            lo, hi = self.short_side_range
            lo = max(lo, self.crop_size)  # 保证可裁
            hi = max(hi, lo)
            target_short = random.randint(lo, hi)
        ...
        if self.infer_policy == "resize256_center":
        ...
        w, h = self._get_size(img)
        short = min(w, h)
        if short <= self.eval_resize_short:
            return F.center_crop(img, [self.crop_size, self.crop_size])
        else:
            img = self._resize_short(img, self.eval_resize_short)
            return F.center_crop(img, [self.crop_size, self.crop_size])
    ...
    def __repr__(self):
        return (
            ...
            f"short_side_range={self.short_side_range}, probs={self.probs}, "
            f"small_image_policy='{self.small_image_policy}', interp={self.interp}, "
            f"antialias={self.antialias}, train={self.train}, "
            f"infer_policy='{self.infer_policy}', eval_resize_short={self.eval_resize_short})"
        )
...
class RandomErasing:
    ...
    def __repr__(self):
        return (
            ...
            + f"(p={self.p}, scale={self.scale}, ratio={self.ratio}, value={self.value})"
        )
...
class RandomMask(object):
    def __init__(self, ratio=0.5, patch_size=14, p=0.5):
        ...
        if isinstance(ratio, float):
            ...
            and len(ratio) == 2
            and all(isinstance(r, float) for r in ratio)
        ):
...
class RandomResizedCropWithVariableSize(transforms.RandomResizedCrop):
    def __init__(
        ...
        self.max_size = max_size
        super().__init__(
            size=min_size, scale=scale, ratio=ratio, interpolation=interpolation
        )
    ...
    def __call__(self, img):
        ...
        ret = F.resized_crop(
            img, i, j, h, w, size, self.interpolation, antialias=self.antialias
        )
        return ret
...
def freq_blend_mix(
    ...
    def rgb_to_hsl(rgb_array):
        ...
        mask = diff != 0
        s[mask & (l <= 0.5)] = (
            diff[mask & (l <= 0.5)] / (max_val + min_val)[mask & (l <= 0.5)]
        )
        s[mask & (l > 0.5)] = (
            diff[mask & (l > 0.5)] / (2.0 - max_val - min_val)[mask & (l > 0.5)]
        )
    ...
    if patch != -1:
        if patch > min(height, width):
            print(
                f"Warning: patch ({patch}) larger than minimum image dimension "
        ...
        for i in range(0, height, step_size):
            for j in range(0, width, step_size):
                ...
                if real_patch.shape != (patch, patch):
                    ...
                    real_patch = np.pad(
                        real_patch, ((0, pad_h), (0, pad_w)), mode="edge"
                    )
                    fake_patch = np.pad(
                        fake_patch, ((0, pad_h), (0, pad_w)), mode="edge"
                    )
                ...
                if mode == "uniform":
                    mixed_dct = (
                        fixed_blend_ratio * real_dct
                        + (1 - fixed_blend_ratio) * fake_dct
                    )
                else:  # variable
                    random_ratios = np.random.uniform(
                        ratios[0], ratios[1], size=real_dct.shape
                    )
                    mixed_dct = (
                        random_ratios * real_dct + (1 - random_ratios) * fake_dct
                    )
    ...
    for channel_idx in range(3):  # All supported color spaces have 3 channels
        ...
        if patch == -1:
            ...
            if mode == "uniform":
                mixed_dct = (
                    fixed_blend_ratio * real_dct + (1 - fixed_blend_ratio) * fake_dct
                )
            else:  # variable
                random_ratios = np.random.uniform(
                    ratios[0], ratios[1], size=real_dct.shape
...
def pixel_blend_mix(
    ...
    if mode == "uniform":
        ...
        alpha_mask = np.random.uniform(
            ratios[0], ratios[1], size=(real_arr.shape[0], real_arr.shape[1], 1)
        )
