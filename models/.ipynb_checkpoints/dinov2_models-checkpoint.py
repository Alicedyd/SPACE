    ...
    def __init__(self, name, num_classes=1, local_files_only=False, model_dir=None, huggingface_path=None):
        ...
        # Try loading from torch hub
            ...
            try:
                ...
                # self.model = torch.hub.load('/inspire/hdd/project/wuliqifa/public/dda/dinov2', name, source='local')
                self.model = torch.hub.load('/inspire/hdd/project/wuliqifa/public/dda/dinov2', name, source='local', pretrained=False)
                ckpt = torch.load("/inspire/hdd/project/wuliqifa/public/dda/dinov2/dinov2_vitl14_pretrain.pth", map_location="cpu")
    ...
    def _load_from_huggingface(self, hf_path, model_name):
        try:
            ...
            if hf_path == "default":
                ...
                model_id = hf_path
            elif hf_path == "/Youtu_Pangu_Security/cusmochen/pretrained/huggingface/models/models--facebook--dinov2-large":
                # Handle the specific path format
                if "dinov2-large" in hf_path and self.model_type != "dinov2_vitl14":
                    print(f"Warning: HuggingFace path specifies dinov2-large but model_type is {self.model_type}")
                model_id = "facebook/dinov2-large"
            elif hf_path == "/Youtu_Pangu_Security/cusmochen/pretrained/huggingface/models/models--facebook--dinov2-with-registers-large":
                # Handle the specific path format for dinov2 with registers
                if self.model_type != "dinov2_vitl14" and self.model_type != "dinov2_vitl14_register":
                    print(f"Warning: HuggingFace path specifies dinov2-with-registers-large but model_type is {self.model_type}")
