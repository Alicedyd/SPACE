...
# 用来添加不固定的实验名称
EXP_ADD="Qwen3-Embedding_Style"

VAE_PATH="/root/autodl-tmp/datasets/StyleCOCO/train2017/fake"
VAE_PATH_ADD="" # 可选的额外 VAE（留空即可）
CHECKPOINTS_DIR="/root/autodl-tmp/codes/ckpt/checkpoints_CLIP_${CROP_SIZE}_LR${LEARNING_RATE}_single_run"
...
if $USE_MULTI_VAE; then
  OPT_FLAGS+=" --multi_vae"
  echo "Will use multiple VAE-reconstructed images in one batch"
fi
...
if $USE_IS_RESIZE; then
  OPT_FLAGS+=" --is_resize"
  echo "Will resize images to cropSize x cropSize"
fi
