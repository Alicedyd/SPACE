...
robust_configs = {
    ...
    "BLUR": [0, 0.5, 1, 1.5, 2],
    "RESIZE": [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2],
}
...
def evaluate_detectors(
    ...
    for subset_name in tqdm(
        ...
        if not (
            ...
            print(os.path.isdir(subset_path_1))
            print(os.path.isdir(subset_path_2))
            print(os.path.isdir(subset_path_3))
            continue
        ...
        for img in all_fake_imgs:
            ...
            # print(f"logit1:{logit1}, logit2:{logit2}, logit3:{logit3}")
...
for robust_type, params in robust_configs.items():
    for param in params:
        print(f"Processing {robust_type} - {param}")

        FOLDER_TRIPLETS = [
            (
                f"/root/autodl-tmp/codes/DDA/result/dda_robustness/{robust_type}_{param}/prediction_results/scores",
                f"/root/autodl-tmp/codes/DDA/result/flux_robustness/{robust_type}_{param}/prediction_results/scores",
                f"/root/autodl-tmp/codes/DDA/result/clip_robustness/{robust_type}_{param}/prediction_results/scores",
            ),
        ]
        ...
        for thresh1, thresh2, thresh3 in product(
            ...
            print(
                f"Processing thresholds: SD={thresh1}, Flux={thresh2}, Model3={thresh3}"
            )
        ...
        if records:
            ...
            final_csv = f"/root/autodl-tmp/codes/DDA/result/merge_results/{robust_type}_{param}_merged.csv"
