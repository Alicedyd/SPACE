...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            "--arch", type=str, default="res50", help="see my_models/__init__.py"
        )
        ...
        parser.add_argument(
            "--real_list_path",
            default=None,
            help="only used if data_mode==ours: path for the list of real images, which should contain train.pickle and val.pickle",
        )
        ...
        parser.add_argument("--vae_models_add", type=str, default=None)

        parser.add_argument("--contrastive", action="store_true")
        parser.add_argument("--use_focal_loss", action="store_true")
        parser.add_argument("--use_logit_adjustment", action="store_true")

        parser.add_argument("--use_randomscale", action="store_true")

        parser.add_argument(
            "--is_resize",
            action="store_true",
            help="if specified, resize the image to cropSize x cropSize",
        )
        ...
        parser.add_argument(
            "--fake_list_path",
            default=None,
            help="only used if data_mode==ours: path for the list of fake images, which should contain train.pickle and val.pickle",
        )
        parser.add_argument("--data_mode", default="ours", help="wang2020 or ours")
        parser.add_argument(
            "--data_label",
            default="train",
            help="label to decide whether train or validation dataset",
        )
        parser.add_argument(
            "--weight_decay", type=float, default=0.0, help="loss weight for l2 reg"
        )
        ...
        parser.add_argument(
            "--local_models_only",
            action="store_true",
            help="只使用本地模型文件，不从网络下载",
        )
        parser.add_argument(
            "--model_dir", type=str, default=None, help="本地模型权重文件的目录路径"
        )
...
class TrainOptions(BaseOptions):
    def initialize(self, parser):
        ...
        parser.add_argument(
            "--meth_pixelmix", type=str, choices=["uniform", "variable"]
        )
        ...
        parser.add_argument("--mix_color_space", type=str, required=True, default="RGB")
